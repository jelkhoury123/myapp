...
spot_exchanges = ['bitstamp','coinbasepro','kraken','liquid','bitfinex','gemini','binance',
                'bittrex','bitbank','huobi','poloniex','bithumb']
exch_dict={}
...
app.layout = html.Div(style={'marginLeft':25,'marginRight':25},
                    children=[ html.Div(className='row',children=[
                                ...
                                dcc.Interval(
                                    ...
                                    interval=10*1000, # in milliseconds= 2 minutes
                                    n_intervals=0
                                    )       
                        ])
                        ])
...
@app.callback(Output('the-data','children'),
            [Input('pairs','value'),Input('interval-component','n_intervals')])
def update_data(pair,n):
    return json.dumps(get_order_books(pair))
...
def update_table(order_books,pair,exchanges,x_scale,y_scale,cutoff,step):
    ...
    return dash_table.DataTable(
        ...
        style_table={'maxHeight': '900','border': 'thin lightgrey solid'},
